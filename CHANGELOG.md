...
- **aiken-lang**: Add `aiken/builtin.unsafe_coerce` for unchecked, zero-cost conversions between types with identical erased opaque representations. Budgeted compile-time checks share the backend's erasure rule and reject incompatible or unconstrained casts; callers remain responsible for opaque-type invariants.
...
- **aiken-project**: save and restore cyclic function indices before and after constant evaluation to avoid free uniques due to reentrancy. @KtorZ
- **aiken-lang**: Report module constant evaluation failures as compiler errors instead of panicking during UPLC generation. @KtorZ
