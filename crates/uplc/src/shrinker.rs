...
fn substitute_var(term: &Term<Name>, original: Name, replace_with: Term<Name>) -> Term<Name> {
    match term {
        Term::Var(name) => {
            if name.text == original.text { replace_with } else { Term::Var(name.clone()) }
        }
        Term::Delay(body) =>
            Term::Delay(Rc::new(substitute_var(body.as_ref(), original, replace_with))),
        Term::Lambda { parameter_name, body } => {
        ...
        Term::Apply { function, argument } =>
            Term::Apply {
                function: Rc::new(
                    substitute_var(function.as_ref(), original.clone(), replace_with.clone())
                ),
