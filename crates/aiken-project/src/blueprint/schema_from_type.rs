    ...
    match tipo.as_ref() {
        Type::App {
            ...
            "List" => {
                let inner = args.first()?;
                let generic = do_build(inner, data_types, type_parameters, definitions)?;
                let data = list_pair_schema_to_map_data(&generic, definitions)
                    .unwrap_or_else(|| Data::List(Items::One(Declaration::Referenced(generic))));
                Some(register_inline_type(
                    &original_tipo,
                    type_parameters,
                    definitions,
...
fn list_pair_schema_to_map_data(
    generic: &Reference,
...
mod tests {
    ...
    fn list_type(inner: Rc<Type>) -> Rc<Type> {
        Type::list(inner)
    }

    fn pair_type(fst: Rc<Type>, snd: Rc<Type>) -> Rc<Type> {
        Type::pair(fst, snd)
    }
    ...
    #[test]
    fn list_of_pairs_produces_map_schema() {
        let dts: IndexMap<DataTypeKey, TypedDataType> = IndexMap::new();
        let refs = aiken_lang::utils::indexmap::as_ref_values(&dts);
        let mut defs = Definitions::new();
        let root = build_schema_from_type(
            &list_type(pair_type(bytearray_type(), int_type())),
            &refs,
            &mut defs,
        )
        .unwrap();
        ...
        match &defs.lookup(&root).unwrap().annotated {
            Schema::Data(Data::Map(keys, values)) => {
                assert!(matches!(keys.schema(&defs), Some(Data::Bytes)));
                assert!(matches!(values.schema(&defs), Some(Data::Integer)));
            }
            other => panic!("expected list of Pair<ByteArray, Int> to lower as map, got {other:?}"),
        }
    }

    #[test]
    fn opaque_wrapper_around_list_of_pairs_produces_map_schema() {
        let mut dts: IndexMap<DataTypeKey, TypedDataType> = IndexMap::new();
        dts.insert(
            DataTypeKey {
                module_name: "crypto".to_string(),
                defined_type: "Assets".to_string(),
            },
            make_opaque_wrapper_type(
                "Assets",
                list_type(pair_type(bytearray_type(), int_type())),
                vec![],
            ),
        ...
        let refs = aiken_lang::utils::indexmap::as_ref_values(&dts);
        let mut defs = Definitions::new();
        let root = build_schema_from_type(&user_app("crypto", "Assets", vec![]), &refs, &mut defs)
            .expect("opaque wrapper containing map should resolve");
        ...
        match &defs.lookup(&root).unwrap().annotated {
            ...
            other => {
                panic!("expected opaque List<Pair<ByteArray, Int>> to lower as map, got {other:?}")
            }
        }
    }
