...
#[test]
fn resolve_type_name_to_type_canonicalizes_qualified_generic_arguments() {
    let data_types = widgets_data_types();

    let result = super::resolve_type_name_to_type(&data_types, "widgets.Result<widgets/State>")
        .expect("slash-qualified generic argument should resolve through canonical fallback");

    let Type::App { args, .. } = result.as_ref() else {
        panic!("Result<widgets/State> should resolve to Type::App")
    };
    let Type::App {
        module, name, args, ..
    } = args[0].as_ref()
    else {
        panic!("generic argument should resolve to Type::App")
    };
...
#[test]
fn collect_inner_data_schemas_preserves_unsupported_pair_cause() {
    let data_types = widgets_data_types();
    let semantics = FuzzerSemantics::DataWithSchema {
        type_name: "Pair<Int, ByteArray>".to_string(),
    };
    ...
    .expect_err("unsupported inner pair schema must surface an error");

    let message = err.to_string();
    assert!(
        message.contains("Pair"),
        "error must preserve pair cause: {message}"
    );
    assert!(
        message.contains("inner_data_schemas"),
        "error must identify inner_data_schemas export: {message}"
    );
}

#[test]
fn transition_inner_data_schema_failures_are_propagated() {
    let data_types = widgets_data_types();
    let prop = aiken_lang::test_framework::TransitionProp::Exists {
        binder: "x".to_string(),
        ty: aiken_lang::test_framework::ShallowIrType::Data,
        domain: Box::new(
            aiken_lang::test_framework::FuzzerSemantics::DataWithSchema {
                type_name: "Pair<Int, ByteArray>".to_string(),
            },
        ),
        body: Box::new(aiken_lang::test_framework::TransitionProp::Unsupported {
            reason: "test body is irrelevant".to_string(),
            source_location: None,
        }),
    };
    let mut inner_data_schemas = std::collections::BTreeMap::new();

    let err = super::extend_inner_data_schemas_from_transition_prop(
        &CheckedModules::default(),
        &data_types,
        &prop,
        "widgets.test",
    ...
    .expect_err("transition inner schema failures must propagate");
    ...
    assert!(inner_data_schemas.is_empty());
}

#[test]
fn export_data_schema_rejects_non_data_pair_nodes() {
    let data_types = widgets_data_types();
    let schema = super::export_data_schema(
        &CheckedModules::default(),
        &data_types,
        &Type::pair(Type::int(), Type::byte_array()),
    );

    let err = schema.expect_err("Pair schemas must be rejected before manifest export");
    assert!(
        err.contains("Pair"),
        "error must identify pair schema: {err}"
    );
}

#[test]
fn exported_data_schema_rejects_map_nodes() {
    let root = crate::blueprint::definitions::Reference::new("MapRoot");
    let mut definitions = crate::blueprint::definitions::Definitions::new();
    definitions.insert(
        &root,
        crate::blueprint::schema::Annotated {
            title: None,
            description: None,
            annotated: crate::blueprint::schema::Schema::Data(crate::blueprint::schema::Data::Map(
                crate::blueprint::schema::Declaration::Inline(Box::new(
                    crate::blueprint::schema::Data::Integer,
                )),
                crate::blueprint::schema::Declaration::Inline(Box::new(
                    crate::blueprint::schema::Data::Bytes,
                )),
            )),
        },
    );
    let schema = crate::export::ExportedDataSchema { root, definitions };

    let err = super::validate_exported_data_schema(&schema)
        .expect_err("Map schemas must be rejected before manifest export");
    assert!(err.contains("Map"), "error must identify map schema: {err}");
}

#[derive(Clone, Copy)]
struct SilentEvents;

impl EventListener for SilentEvents {}

#[test]
fn export_tests_errors_on_uncompiled_project() {
    let package = PackageName {
        owner: "test".to_string(),
        repo: "project".to_string(),
    };
    let config = ProjectConfig::default(&package);
    let mut project = super::Project::new_with_config(config, PathBuf::new(), SilentEvents);

    let err = project
        .export_tests(None, false, Tracing::silent(), false)
        .expect_err("fresh project must not silently export an empty manifest");
    let message = err.to_string();
    assert!(
        message.contains("requires a compiled project"),
        "fresh-project error should tell callers to compile first: {message}"
    );
}
