...
fn convert_constraint(lang: &LangFuzzerConstraint) -> FuzzerConstraint {
    match lang {
        ...
        LangFuzzerConstraint::Empty { reason } => FuzzerConstraint::Empty {
            reason: reason.clone(),
        },
...
fn schema_type_name_candidates(type_name: &str) -> Vec<String> {
    ...
    let Some((head, arg_keys)) = split_type_application(type_name) else {
        return candidates;
    };
    let suffix = if arg_keys.is_empty() {
        String::new()
    } else {
        format!("<{}>", arg_keys.join(", "))
    };

    if let Some((module, name)) = head.rsplit_once('.') {
        let slash = format!("{module}/{name}{suffix}");
        if !candidates.contains(&slash) {
            candidates.push(slash);
        }
    }
    if let Some((module, name)) = head.rsplit_once('/') {
        let dotted = format!("{module}.{name}{suffix}");
...
fn insert_inner_data_schema_with_dependencies(
    ...
    let schema = export_data_schema(modules, data_types, &tipo)
        .map_err(|cause| {
            Error::StandardIo(std::io::Error::other(format!(
                "Test '{test_name}': DataWithSchema leaf '{type_name}' could not be exported to inner_data_schemas: {cause}"
            )))
        })?
        .ok_or_else(|| {
            Error::StandardIo(std::io::Error::other(format!(
...
fn resolve_type_name_to_type(
    ...
    schema_type_name_candidates(type_name)
        .into_iter()
        .find_map(|candidate| resolve_type_name_to_type_candidate(data_types, &candidate))
}

fn resolve_type_name_to_type_candidate(
    data_types: &IndexMap<DataTypeKey, TypedDataType>,
    type_name: &str,
) -> Option<Rc<Type>> {
...
fn validate_exported_data_schema(schema: &ExportedDataSchema) -> Result<(), String> {
    let root = schema
        .definitions
        .try_lookup(&schema.root)
        .ok_or_else(|| format!("schema root '{}' is unresolved", schema.root.as_key()))?;
    validate_exported_schema_node(&schema.root.as_key(), &root.annotated)?;

    for (key, annotated) in schema.definitions.iter() {
        validate_exported_schema_node(key, &annotated.annotated)?;
    }

    Ok(())
}

fn validate_exported_schema_node(key: &str, schema: &Schema) -> Result<(), String> {
    match schema {
        Schema::Data(data) => validate_exported_schema_data(key, data),
        Schema::Unit => Err(format!(
            "schema definition '{key}' is Unit, but verification export only supports Data schemas"
        )),
        Schema::Boolean => Err(format!(
            "schema definition '{key}' is Boolean, but verification export only supports Data schemas"
        )),
        Schema::Integer => Err(format!(
            "schema definition '{key}' is Integer, but verification export only supports Data schemas"
        )),
        Schema::Bytes => Err(format!(
            "schema definition '{key}' is Bytes, but verification export only supports Data schemas"
        )),
        Schema::String => Err(format!(
            "schema definition '{key}' is String, but verification export only supports Data schemas"
        )),
        Schema::Pair(_, _) => Err(format!(
            "schema definition '{key}' is Pair, but verification export only supports Data schemas"
        )),
        Schema::List(_) => Err(format!(
            "schema definition '{key}' is List, but verification export only supports Data schemas"
        )),
    }
}

fn validate_exported_schema_declaration(
    key: &str,
    declaration: &BlueprintSchemaDeclaration<BlueprintSchemaData>,
) -> Result<(), String> {
    if let BlueprintSchemaDeclaration::Inline(data) = declaration {
        validate_exported_schema_data(key, data)?;
    }
    Ok(())
}

fn validate_exported_schema_data(key: &str, data: &BlueprintSchemaData) -> Result<(), String> {
    match data {
        BlueprintSchemaData::Integer | BlueprintSchemaData::Bytes | BlueprintSchemaData::Opaque => {
            Ok(())
        }
        BlueprintSchemaData::List(BlueprintSchemaItems::One(item)) => {
            validate_exported_schema_declaration(key, item)
        }
        BlueprintSchemaData::List(BlueprintSchemaItems::Many(items)) => {
            for item in items {
                validate_exported_schema_declaration(key, &item.annotated)?;
            }
            Ok(())
        }
        BlueprintSchemaData::Map(_, _) => Err(format!(
            "schema definition '{key}' contains Map, but verification export does not support Map schemas"
        )),
        BlueprintSchemaData::AnyOf(constructors) => {
            for constructor in constructors {
                for field in &constructor.annotated.fields {
                    validate_exported_schema_declaration(key, &field.annotated)?;
                }
            }
            Ok(())
        }
...
fn export_data_schema(
    ...
    data_types: &IndexMap<DataTypeKey, TypedDataType>,
    tipo: &Rc<Type>,
) -> Result<Option<ExportedDataSchema>, String> {
    ...
    if let Ok(root) = Annotated::from_type(modules.into(), tipo.as_ref(), &mut definitions) {
        let schema = ExportedDataSchema { root, definitions };
        validate_exported_data_schema(&schema)?;
    ...
    let Some(root) = blueprint::schema_from_type::build_schema_from_type(
        tipo,
        &data_type_refs,
    ...
    T: EventListener,
{
    ...
    pub fn export_tests(
        ...
        if self.checked_modules.is_empty() {
            return Err(Error::StandardIo(std::io::Error::other(
                "Project::export_tests requires a compiled project; call Project::compile before exporting tests.",
            )));
        }
    ...
    fn export_property_test(
        ...
        .map_err(|cause| {
            Error::StandardIo(std::io::Error::other(format!(
                "Test '{}.{}': fuzzer data schema could not be exported: {cause}",
    ...
    fn type_check(
        ...
        let mut next_glossary = self.glossary.clone();
        modules.extends_glossary(&mut next_glossary);
