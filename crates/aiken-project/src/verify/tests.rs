...
fn derive_fixture_semantics_from_constraint(
    ...
    match constraint {
        ...
        FuzzerConstraint::Empty { reason } => FuzzerSemantics::Opaque {
            reason: format!("empty fuzzer support: {reason}"),
        },
...
fn generate_workspace_creates_files() {
    ...
    let config = VerifyConfig {
        trust_profile: TrustProfile::Production,
...
fn equivalence_theorem_name_for_entry_detects_marker_in_proof_file() {
    ...
    assert_eq!(
        equivalence_theorem_name_for_entry(&entry),
...
fn equivalence_theorem_name_for_entry_returns_none_without_marker() {
    let entry = ManifestEntry {
        ...
        lean_module: "AikenVerify.Proofs.My_module.test_eq".to_string(),
        lean_theorem: "test_eq".to_string(),
        lean_file: "AikenVerify/Proofs/My_module/test_eq.lean".to_string(),
        flat_file: "cbor/my_module__test_eq_deadbeef.cbor".to_string(),
        fuzzer_flat_file: None,
        return_mode: None,
        test_mode: None,
    ...
    assert_eq!(equivalence_theorem_name_for_entry(&entry), None);
}
...
fn parse_verify_results_all_proved() {
    ...
    let summary = parse_verify_results(raw, &manifest, VerifyParseContext::for_tests());
...
fn verify_summary_version_is_current() {
    // Wire-version assertions must use the literal value expected by external
    // JSON consumers, not only the producer constant. This fails loud when a
    // result-shape change forgets to bump the public schema version.
    const EXPECTED_VERIFY_SUMMARY_VERSION: &str = "4";
    assert_eq!(VERIFY_SUMMARY_VERSION, EXPECTED_VERIFY_SUMMARY_VERSION);
    ...
    let summary = parse_verify_results(raw, &manifest, VerifyParseContext::for_tests());
    assert_eq!(
        summary.verify_summary_version,
        EXPECTED_VERIFY_SUMMARY_VERSION
    );

    let json = serde_json::to_value(&summary).expect("summary should serialize");
    assert_eq!(
        json["verify_summary_version"],
        serde_json::Value::String(EXPECTED_VERIFY_SUMMARY_VERSION.to_string())
    );
}
...
fn verify_summary_includes_blaster_rev_and_plutus_core_rev() {
    ...
    let mut summary = parse_verify_results(raw, &manifest, VerifyParseContext::for_tests());
...
fn lean_version_constant_matches_pinned_version() {
    ...
    assert!(
        workflow.contains(&format!("LEAN_VERSION: \"{}.{}.{}\"", maj, min, patch)),
        "tests workflow LEAN_VERSION must agree with MIN_LEAN_VERSION/lean-toolchain literal"
    );
...
fn exact_solver_counterexample_replays_to_confirmed_aiken_input() {
    ...
    manifest.execution.cek_budget.cpu = Some(ExBudget::default().cpu.to_string());
    manifest.execution.cek_budget.mem = Some(ExBudget::default().mem.to_string());
    ...
    assert_eq!(result.certification, Certification::SmtCounterexample);
...
#[test]
fn parse_verify_results_trust_reapply_preserves_replay_counterexample() {
    let tmp = tempfile::tempdir().unwrap();
    let rel =
        write_counterexample_property_program(tmp.path(), "trust_reapply_counterexample", false);
    let mut manifest = make_manifest(vec![(
        "example",
        "trust_reapply_counterexample",
        "trust_reapply_counterexample",
    )]);
    manifest.tests[0].flat_file = rel;
    manifest.execution.cek_budget.cpu = Some(ExBudget::default().cpu.to_string());
    ...
    let mut theorem = TheoremResult::from_manifest_entry_with_replay(
        "example.trust_reapply_counterexample".to_string(),
        manifest.tests[0].lean_theorem.clone(),
        &manifest.tests[0],
        ProofStatus::Failed {
            category: FailureCategory::Counterexample,
    ...
    theorem.trust_profile = TrustProfile::UnsafeDev;
    ...
    let raw = VerifyResult {
        ...
        stderr: CapturedOutput::small(""),
        exit_code: Some(1),
        theorem_results: Some(vec![theorem]),
    };
    let mut context = VerifyParseContext::for_tests();
    context.trust_profile = TrustProfile::Strict;

    let summary = parse_verify_results(raw, &manifest, context);
    assert_eq!(summary.total, 1);
    assert_eq!(summary.failed, 1);

    let result = &summary.theorems[0];
    assert_eq!(result.trust_profile, TrustProfile::Strict);
    assert_eq!(result.status, VerificationStatus::SolverFalsified);
    assert_eq!(result.certification, Certification::SmtCounterexample);
    assert!(matches!(
        result.proof_status,
    ...
    let counterexample = result
        .counterexample
        .as_ref()
        .expect("confirmed replay counterexample must be preserved");
    assert_eq!(
        counterexample.replay_status,
        CounterexampleReplayStatus::Confirmed
    );
    ...
    assert!(
        counterexample.replay_note.is_some(),
        "replay note must survive trust-profile restamping"
    );
}
...
fn certification_serializes_all_evidence_classes() {
    let cases = [
        ...
        (Certification::SmtCounterexample, "smt_counterexample"),
...
fn legacy_proved_status_deserializes_as_solver_validated() {
    ...
    let ProofStatus::Partial { note } = &result.proof_status else {
        panic!(
            "legacy success must conservatively deserialize as Partial, got {:?}",
            result.proof_status
        );
    };
    assert!(
        note.starts_with("Legacy theorem result omitted trust_profile; success cannot be reassessed against the selected trust policy."),
        "legacy downgrade note must name the omitted trust profile, got: {note}"
    );
    assert!(
        note.contains("Domain metadata blocks production success"),
        "legacy downgrade note must include reassessment detail, got: {note}"
    );
    assert_eq!(result.status, VerificationStatus::Partial);
...
#[test]
fn solver_falsified_deserialization_uses_counterexample_certification() {
    let value = serde_json::json!({
        "test_name": "mod.test",
        "theorem_name": "test",
        "status": "SolverFalsified",
        "trust_profile": "production",
        "domain": trusted_overapprox_domain()
    });

    let result: TheoremResult =
        serde_json::from_value(value).expect("solver-falsified theorem result should deserialize");
    ...
    assert_eq!(
        result
            .counterexample
            .as_ref()
            .expect("counterexample metadata should be rebuilt")
            .classification,
        CounterexampleClassification::Potential
    );
}
...
#[test]
fn verify_summary_missing_version_defaults_to_current_literal() {
    let value = serde_json::json!({
        "total": 0,
        "proved": 0,
        "partial": 0,
        ...
        "raw_output": {
            "success": true,
            "stdout": { "tail": "", "total_bytes": 0, "truncated": false },
    ...
    let summary: VerifySummary =
        serde_json::from_value(value).expect("legacy summary should deserialize");
    assert_eq!(summary.verify_summary_version, "4");
}

#[test]
fn reapply_manifest_trust_profile_recovers_original_success_and_recomputes_counts() {
    let mut manifest = make_manifest(vec![("mod", "test", "test")]);
    manifest.tests[0].domain = Some(trusted_overapprox_domain());
    manifest.tests[0].test_mode = Some(ManifestTestMode::Normal);
    manifest.tests[0].return_mode = Some(TestReturnMode::Bool);

    let strict_result = TheoremResult::from_manifest_entry_with_profile(
        "mod.test".to_string(),
        "test".to_string(),
        &manifest.tests[0],
        ProofStatus::Proved,
    ...
    assert_eq!(strict_result.status, VerificationStatus::Partial);

    let raw = VerifyResult::new(
        true,
    ...
    reapply_manifest_trust_profile(&mut summary, &manifest, TrustProfile::Production);
    ...
    assert_eq!(
        ...
        VerificationStatus::SolverValidated
    );
    assert_eq!(
        summary.theorems[0].certification,
        Certification::SmtValidNoProofReconstruction
    );
}
...
#[test]
fn witness_proved_explanation_mentions_concrete_instances_only() {
    let result = TheoremResult::new_with_domain(
        "sm_module.halt_only".to_string(),
        "halt_only".to_string(),
    ...
    let explanation = result
        .explanation
        .as_deref()
        .expect("WitnessProved must explain its non-universal scope");
    assert!(
        explanation.contains("single-witness native_decide proof"),
        "explanation must include the witness note, got: {explanation}"
    );
    assert!(
        explanation.contains("listed concrete instance(s), not a universal theorem"),
        "explanation must clarify witness-only scope, got: {explanation}"
    );
}
...
fn parse_verify_results_marks_witness_entries_as_witness_proved_on_success() {
    ...
    // must surface it as `WitnessProved` rather than `Proved`. This test
    // confirms the wire path works end-to-end by setting the field manually.
...
#[test]
fn primitive_filtered_bool_domain_is_exact_when_predicate_lowers() {
    let (_tmp, exported) = export_verify_primitive_lowering_fixture();
    ...
    assert!(
        ...
        "lowered such_that predicates must not leave a domain widening"
    );
}
...
fn primitive_fixture_hashes_are_pinned() {
    ...
    let expected = [
        ...
        (
            "prop_bool_filtered",
            "6bd976e9048f0437aef83bb22e85023b57d9c34894b5fe56e2033edf04daa1e3",
        ),
        (
            "prop_byte",
...
fn composed_one_of_and_filter_domains_are_visible_and_accept_real_samples() {
    ...
    match &filter_domain.relation {
        DomainRel::Filter {
            ...
            assert!(predicate_ir.is_some());
        }
        other => panic!("expected filter relation, got {other:?}"),
    }
    assert_eq!(filter_domain.precision, DomainPrecision::Exact);
    for seed in [0_u32, 1, 42, 1337] {
...
fn equivalence_target_mode_uses_non_vacuous_void_equivalence_goal() {
    ...
    assert!(
        proof.contains("theorem test_eq_void_equivalence :"),
        "Void equivalence should generate the canonical equivalence theorem, got:\n{proof}"
    );
    assert!(
        proof.contains("proveTestsHalt") && proof.contains(lean_sym::IFF),
        "Void equivalence theorem must compare halt semantics, got:\n{proof}"
    );
}
...
fn compat_all_supported_scalar_types_generate_workspace() {
    let types_and_constraints = [
        ...
        (
            FuzzerOutputType::Bool,
            FuzzerConstraint::Exact(FuzzerExactValue::Bool(false)),
        ),
        (
            FuzzerOutputType::String,
            FuzzerConstraint::Exact(FuzzerExactValue::String("Hi".to_string())),
    ...
    for (i, (output_type, constraint)) in types_and_constraints.iter().enumerate() {
        ...
        let manifest =
            generate_lean_workspace(&[test], &config, &SkipPolicy::All).unwrap_or_else(|e| {
                panic!(
                    "Scalar type {:?} should generate workspace without skips, got: {e}",
                    output_type
                )
            });
        ...
        assert_eq!(
            manifest.tests.len(),
            1,
            "Scalar type {:?} must emit one proof entry, got tests={} skipped={}",
            output_type,
            manifest.tests.len(),
            manifest.skipped.len()
        );
        assert!(
            ...
            "Scalar type {:?} must not regress from proof emission to skip output, got {:?}",
            output_type,
            manifest.skipped
        );
    }
...
fn scenario_witness_trace_domain_records_replay_metadata() {
    ...
    assert!(
        domain
            .obligations_discharged
            .contains(&DomainObligation::WitnessReplaysThroughFuzzer)
    );
    assert!(
        domain
            .obligations_discharged
            .contains(&DomainObligation::WitnessSatisfiesDomain)
    );
}
...
fn phase12_scenario_matrix_collects_trace_status_and_blockers() {
    ...
    assert_eq!(
        rows[1].symbolic_status,
        VerificationStatus::WitnessValidated
    );
...
fn dict_type_flows_through_as_list_of_pairs() {
    ...
    assert!(
        proof.contains("let (p_0_1, p_0_2) := x_i; Data.List [p_0_1, Data.I p_0_2]"),
        "Dict pair lowering must preserve pair/component encoding inside the list-backed representation, got:\n{proof}"
    );
    assert!(
        proof.contains("xs.length"),
...
#[test]
fn emit_shallow_match_arm_guard_constrains_refined_constructor_pattern() {
    use aiken_lang::test_framework::{
        ShallowBinOp, ShallowConst, ShallowIr, ShallowIrArm, ShallowIrType, TransitionProp,
    };

    let subject = ShallowIr::BoundVar {
        name: "maybe".to_string(),
        ty: ShallowIrType::Data,
    };
    let some_zero_guard = ShallowIr::BinOp {
        op: ShallowBinOp::Eq,
        left: Box::new(subject.clone()),
        right: Box::new(ShallowIr::Construct {
            module: "fixture".to_string(),
            constructor: "Some".to_string(),
            tag: 1,
            fields: vec![ShallowIr::Const(ShallowConst::Int("0".to_string()))],
        }),
    };
    let prop = TransitionProp::EqOutput(ShallowIr::Match {
        subject: Box::new(subject),
        arms: vec![
            ShallowIrArm {
                tag: Some(1),
                bindings: vec!["_".to_string()],
                guard: Some(Box::new(some_zero_guard)),
                body: ShallowIr::Const(ShallowConst::Int("10".to_string())),
            },
            ShallowIrArm {
                tag: None,
                bindings: vec!["_".to_string()],
                guard: None,
                body: ShallowIr::Const(ShallowConst::Int("20".to_string())),
            },
        ],
    });

    let (def, log, _sub_gens, s0002) =
        crate::verify::emit_is_valid_transition_def_for_export("guarded_match_ivt", &prop);

    assert!(log.is_empty(), "guarded match should emit exactly: {log:?}");
    assert!(s0002.is_none(), "no S0002 marker expected: {s0002:?}");
    assert!(
        def.contains("decide (tag = (1 : Integer)) && decide"),
        "constructor arm must combine tag and refinement guard, got:\n{def}"
    );
    assert!(
        def.contains("Data.Constr (1 : Integer) [Data.I (0 : Integer)]"),
        "refinement guard must include the Some(0) payload, got:\n{def}"
    );
    assert!(
        def.contains("Data.I (10 : Integer)") && def.contains("Data.I (20 : Integer)"),
        "both guarded and fallback arms should remain present, got:\n{def}"
    );
}
...
fn emit_transition_prop_permissions_style_step_output_stays_exact() {
    ...
    assert!(
        !log.iter().any(|entry| entry.contains("freshened")),
        "permissions-style step output should stay structural without freshening, got: {log:?}"
    );
    assert!(
...
fn state_machine_halt_with_step_ir_but_no_transition_prop_and_no_witnesses_is_skipped() {
    ...
    assert_proof_generation_skipped_as_fallback(
        ...
        "no extractable transition constraints",
    );
}
...
fn state_machine_halt_two_phase_off_guard_does_not_block_when_witnesses_present() {
    ...
    // the no-witness bypass guard must not fire. Instead the halt test is
    // routed to the concrete witness proof path and surfaced as WitnessProved.
    use aiken_lang::test_framework::{ShallowIr, ShallowIrType};
    ...
    } = test.semantics
    {
        ...
        *output_semantics = Box::new(success_trace_output_semantics());
    }
    test.transition_prop_lean = Some(crate::export::ExportedTransitionProp {
        is_valid_transition_def:
            "def t_isValidTransition (state : Data) (transition : Data) : Prop := True".to_string(),
    ...
    let (content, caveat) = result.expect("witness-backed halt proof must be generated");
    assert!(
        content.contains("private def sample_0 : Data"),
        "halt witness proof must emit a concrete sample definition, got:\n{content}"
    );
    assert!(
        content.contains("proveTestsHalt") && content.contains("native_decide"),
        "halt witness proof must use native_decide over proveTestsHalt, got:\n{content}"
    );
    ...
    match caveat {
        ProofCaveat::Witness(WitnessProofNote {
            ...
            assert_eq!(witnesses, vec!["80".to_string()]);
            assert!(
                note.contains("state-machine halt test 'permissions.test.prop_two_phase_off_with_witnesses_ok'"),
                "note must mention the test name and halt path, got: {note}"
            );
            assert!(
                note.contains("1 concrete fuzzer-seed-42 witness only"),
                "note must include the concrete witness count and base seed, got: {note}"
            );
            assert!(
                note.contains("universal proof of `∀ x, reachable x → proveTestsHalt"),
                "note must state that no universal reachable theorem is claimed, got: {note}"
            );
        }
        other => panic!("expected ProofCaveat::Witness, got {other:?}"),
    }
...
#[test]
fn state_machine_halt_workspace_preserves_witness_caveat_when_two_phase_off() {
    // Workspace generation computes compatibility metadata before the final
    // proof route is known. A witness-backed halt proof must remain witness-only
    // in the manifest instead of being downgraded by a universal-route blocker.
    use aiken_lang::test_framework::{ShallowIr, ShallowIrType};
    ...
    let out_dir = tmp.path().to_path_buf();
    let mut test = make_phase12_state_machine_test(
        "prop_two_phase_off_workspace_witness_ok",
        StateMachineAcceptance::AcceptsSuccess,
    );
    if let FuzzerSemantics::StateMachineTrace {
        ref mut step_function_ir,
    ...
    let result = generate_lean_workspace(&[test], &config, &SkipPolicy::None);
    ...
    let manifest = result.expect("workspace generation should preserve witness caveat");
    assert_eq!(manifest.tests.len(), 1);
    let entry = &manifest.tests[0];
    assert!(
        entry.partial_proof_note.is_none(),
        "witness halt proof must not be downgraded to Partial: {:?}",
        entry.partial_proof_note
    );
    let witness_note = entry
        .witness_proof_note
        .as_ref()
        .expect("workspace manifest must retain witness proof note");
    assert_eq!(witness_note.instances, 1);
    assert_eq!(witness_note.witnesses, vec!["80".to_string()]);
    assert!(
        witness_note
            .note
            .contains("1 concrete fuzzer-seed-42 witness only"),
        "witness note must describe concrete-only scope, got: {}",
        witness_note.note
    );
    ...
    assert!(proof_content.contains("private def sample_0 : Data"));
    assert!(proof_content.contains("proveTestsHalt") && proof_content.contains("native_decide"));
    ...
    assert_eq!(summary.partial, 0);
    assert_eq!(summary.proved, 0);
    let theorem = summary.theorems.first().expect("witness theorem present");
    assert_eq!(theorem.status, VerificationStatus::WitnessValidated);
    assert_eq!(theorem.certification, Certification::WitnessReplay);
}
...
fn two_phase_domain_only_widening_remains_partial() {
    ...
    match caveat {
        ProofCaveat::Partial(note) => {
            assert!(note.contains("generated trace predicate is non-sequential"));
