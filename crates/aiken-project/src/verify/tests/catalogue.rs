...
use super::super::error_catalogue::{CATALOGUE, EXPOSED_CATALOGUE_CODES, unsupported};
...
/// `every_unsupported_reason_has_catalogue_entry`,
/// `all_reason_witnesses_cover_every_runtime_catalogue_code`, and
...
#[test]
fn all_reason_witnesses_cover_every_runtime_catalogue_code() {
    let reason_codes: BTreeSet<&'static str> = all_reasons().iter().map(code_for_reason).collect();
    let catalogue_codes: BTreeSet<&'static str> = EXPOSED_CATALOGUE_CODES.iter().copied().collect();
    assert!(
        catalogue_codes.is_subset(&reason_codes),
        "all_reasons/code_for_reason must provide a representative witness for every exposed runtime code"
    );
}
...
fn every_catalogue_entry_has_metadata_populated() {
    ...
    for entry in CATALOGUE {
        ...
        if EXPOSED_CATALOGUE_CODES.contains(&entry.code) {
            let reason = all_reasons()
                .into_iter()
                .find(|reason| code_for_reason(reason) == entry.code)
                .expect("every exposed catalogue entry must have a representative reason");
            let serialized_reason =
                serde_json::to_value(reason).expect("unsupported reason should serialize");
            let feature_tag = serialized_reason["code"]
                .as_str()
                .expect("serialized reason tag should be a string");
            assert_eq!(
                entry.feature, feature_tag,
                "catalogue feature slug for code `{}` must match the serialized UnsupportedReason tag",
                entry.code
            );
        }
