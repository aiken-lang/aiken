...
use uplc::{
    BoundedBytes, Constr, KeyValuePairs, MaybeIndefArray, PlutusData,
...
pub enum Certification {
    ...
    /// SMT/Blaster produced a falsifying model/counterexample for the generated theorem.
    SmtCounterexample,
...
impl Certification {
    fn from_proof_status(status: &ProofStatus) -> Self {
        match status {
            ...
            ProofStatus::Failed { category, .. } => match category {
                FailureCategory::Counterexample => Certification::SmtCounterexample,
...
impl std::fmt::Display for CounterexampleReplayStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            ...
            CounterexampleReplayStatus::ReadFailed => "read_failed",
            CounterexampleReplayStatus::HarnessFailed => "harness_failed",
...
fn default_verify_summary_version() -> String {
    VERIFY_SUMMARY_VERSION.to_string()
}
...
pub struct VerifySummary {
    ...
    #[serde(default = "default_verify_summary_version")]
    pub verify_summary_version: String,
}
impl VerifySummary {
    pub fn no_proofs(
...
pub enum DomainRel {
    ...
    Filter {
        ...
        #[serde(default, skip_serializing_if = "Option::is_none")]
        predicate_ir: Option<aiken_lang::test_framework::ShallowIr>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
...
fn theorem_result_explanation(
    ...
    match proof_status {
        ...
        ProofStatus::WitnessProved { note, .. } => {
            let generic = "Witness validation proves only the listed concrete instance(s), not a universal theorem.";
            clean_reason(note)
...
impl<'de> serde::Deserialize<'de> for TheoremResult {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let (wire_status, proof_status) = match wire.status {
        ...
        let explicit_certification = wire.certification;
        let missing_trust_profile = wire.trust_profile.is_none();
        let trust_profile = wire.trust_profile.unwrap_or_else(default_trust_profile);
        let domain = wire
            .domain
            .unwrap_or_else(LoweredDomain::legacy_placeholder);
        let input_type = wire.input_type;
        let original_success = proof_status_is_success(&proof_status);

        let mut result = TheoremResult::new_with_domain(
            wire.test_name,
            wire.theorem_name,
            proof_status,
            wire.over_approximations,
            trust_profile,
            domain,
            input_type,
            None,
        ...
        if wire_status == VerificationStatus::LeanCertified
            && matches!(result.proof_status, ProofStatus::Proved)
            && result.status == VerificationStatus::SolverValidated
        {
            result.status = VerificationStatus::LeanCertified;
            result.certification =
                explicit_certification.unwrap_or(Certification::LeanKernelChecked);
        } else if let Some(certification) = explicit_certification {
            let stale_counterexample_certification = wire_status
                == VerificationStatus::SolverFalsified
                && certification == Certification::SmtValidNoProofReconstruction;
            if !stale_counterexample_certification {
                result.certification = certification;
            }
        }

        let mut rebuilt_explanation = wire.explanation.is_none();
        if missing_trust_profile && original_success {
            let note = legacy_missing_trust_profile_note(&result.proof_status);
            result.set_proof_status(ProofStatus::Partial { note });
            rebuilt_explanation = true;
        }

        if let Some(counterexample) = wire.counterexample {
            result.counterexample = Some(counterexample);
            result.reclassify_counterexample();
            rebuilt_explanation = true;
        }
        result.metrics = wire.metrics;

        if rebuilt_explanation {
            result.rebuild_explanation();
        } else {
            result.explanation = wire.explanation;
        }
...
fn proof_status_is_success(status: &ProofStatus) -> bool {
    matches!(
        status,
        ProofStatus::Proved | ProofStatus::WitnessProved { .. } | ProofStatus::Partial { .. }
    )
}

fn proof_status_is_domain_downgrade(status: &ProofStatus) -> bool {
    matches!(
        status,
        ProofStatus::Partial { note } if note.starts_with("Domain metadata blocks ")
            || note.starts_with("Legacy theorem result omitted trust_profile")
    )
}

fn legacy_missing_trust_profile_note(existing: &ProofStatus) -> String {
    let legacy = "Legacy theorem result omitted trust_profile; success cannot be reassessed against the selected trust policy.";
    match existing {
        ProofStatus::Partial { note } if !note.trim().is_empty() && !note.contains(legacy) => {
            format!("{legacy} {note}")
        }
        _ => legacy.to_string(),
    }
...
impl TheoremResult {
    ...
    fn refresh_counterexample(&mut self, replay_context: Option<CounterexampleReplayContext<'_>>) {
        ...
        self.reclassify_counterexample();
        self.rebuild_explanation();
    }

    fn restamp_trust_profile_preserving_observed_failure(&mut self, trust_profile: TrustProfile) {
        self.trust_profile = trust_profile;
        self.status = VerificationStatus::from_proof_status(&self.proof_status);
        self.certification = Certification::from_proof_status(&self.proof_status);
        self.reclassify_counterexample();
        self.rebuild_explanation();
    }

    fn reclassify_counterexample(&mut self) {
        if !matches!(
            self.proof_status,
            ProofStatus::Failed {
                category: FailureCategory::Counterexample,
        ...
        self.certification = Certification::SmtCounterexample;
        self.status = if self.counterexample.as_ref().is_some_and(|counterexample| {
            counterexample.classification == CounterexampleClassification::Potential
        }) {
    ...
    pub fn from_manifest_entry_with_replay_and_profile(
        ...
        if theorem_name == entry.lean_theorem {
            result.refresh_counterexample(Some(CounterexampleReplayContext {
                ...
                ex_budget: ex_budget_from_manifest(manifest),
            }));
        }
        result
    }
...
fn manifest_success_status_for_theorem(
    entry: &ManifestEntry,
    theorem_name: &str,
    current_status: &ProofStatus,
    current_over_approximations: usize,
) -> (ProofStatus, usize) {
    if theorem_name == entry.lean_theorem {
        return (
    ...
    if theorem_name == format!("{}_alwaysTerminating", entry.lean_theorem)
        || theorem_name == format!("{}_equivalence", entry.lean_theorem)
    {
        return (ProofStatus::Proved, 0);
    }

    (current_status.clone(), current_over_approximations)
}

pub(super) fn proof_counts(
    ...
    let unknown = theorems
        ...
        .count();
    (proved, partial, witness, failed, timed_out, unknown)
}

fn recompute_summary_proof_counts(summary: &mut VerifySummary) {
    let (proved, partial, witness, failed, timed_out, unknown) = proof_counts(&summary.theorems);
    summary.total = summary.theorems.len();
    ...
    summary.command_success = failed == 0 && timed_out == 0 && unknown == 0;
}
...
pub fn reapply_manifest_trust_profile(
    ...
    for theorem in &mut summary.theorems {
        let Some(entry) = manifest_entry_for_theorem(manifest, theorem) else {
            theorem.trust_profile = trust_profile;
            continue;
        };

        if proof_status_is_success(&theorem.proof_status)
            || proof_status_is_domain_downgrade(&theorem.proof_status)
        {
            let (proof_status, over_approximations) = manifest_success_status_for_theorem(
                entry,
            ...
            *theorem = TheoremResult::from_manifest_entry_with_profile(
            ...
            theorem.restamp_trust_profile_preserving_observed_failure(trust_profile);
        }
    }

    recompute_summary_proof_counts(summary);
}
...
pub struct VerifyConfig {
    ...
    /// Trust profile used when assessing generated domain diagnostics.
    pub trust_profile: TrustProfile,
    ...
    /// `VerifyResult`. Command capture is always bounded to
    /// `MAX_RAW_OUTPUT_TAIL_BYTES` before buffering; the last
    /// `raw_output_bytes` bytes are then kept in JSON and in the diagnostic
    /// log artifact. `0` disables the second JSON/log truncation step only.
...
fn manifest_entry_caveat_with_soundness_lint(
    ...
    let Ok(proof_content) = fs::read_to_string(&proof_path) else {
        return append_partial_caveat_note(
            caveat,
            format!(
                "generated proof artifact {} could not be read; result downgraded because soundness linting could not inspect the Lean file",
                proof_path.display()
            ),
        );
    };
...
fn ex_budget_from_manifest(manifest: &GeneratedManifest) -> ExBudget {
    fn parse_i64(value: Option<&String>) -> Option<i64> {
        value.and_then(|text| text.parse::<i64>().ok())
    }
    ...
    let fuel = manifest
        ...
        .unwrap_or_else(|| ExBudget::default().cpu);
    ExBudget {
        mem: parse_i64(manifest.execution.cek_budget.mem.as_ref()).unwrap_or(fuel),
        cpu: parse_i64(manifest.execution.cek_budget.cpu.as_ref()).unwrap_or(fuel),
    }
}
...
fn entry_requires_sampler_metadata(entry: &ManifestEntry) -> bool {
    entry.domain.as_ref().is_some_and(|domain| {
        matches!(domain.relation, DomainRel::SamplerReturns { .. })
            || (matches!(domain.relation, DomainRel::Witness { .. })
                && domain.certificate == DomainCertificate::WitnessReplay)
...
fn structure_source_span(structure: &ExportedFuzzerStructure) -> Option<ExportedSourceSpan> {
    match structure {
        ...
        | ExportedFuzzerStructure::Empty { source_span, .. }
...
fn structure_support_upper_bound(structure: &ExportedFuzzerStructure) -> Option<usize> {
    match structure {
        ExportedFuzzerStructure::Primitive {
            ...
            Some(FuzzerConstraint::Empty { .. }) => Some(0),
...
fn primitive_structure_profile(
    ...
    match known_constraint {
        Some(FuzzerConstraint::Empty { reason }) => RelationalStructureProfile {
            relation: DomainRel::Constraint {
                constraint: FuzzerConstraint::Empty {
                    reason: reason.clone(),
                },
            },
            precision: DomainPrecision::Exact,
            diagnostics: vec![format!("Fuzzer support is empty: {reason}")],
        ...
        ExportedFuzzerStructure::Filter {
            ...
            RelationalStructureProfile {
                relation: DomainRel::Filter {
                    ...
                    predicate_ir: predicate_ir.clone(),
                    max_tries: *max_tries,
...
fn downgrade_precision_for_constraint_gap(
    precision: DomainPrecision,
    constraint_gap_active: bool,
) -> DomainPrecision {
    if constraint_gap_active && precision == DomainPrecision::Exact {
        DomainPrecision::OverApprox
    } else {
        precision
    }
...
fn exact_semantic_existential_support_available(
    ...
    if let Some(profile) = &structured_profile {
        let precision =
            downgrade_precision_for_constraint_gap(profile.precision, constraint_gap_active);
...
fn lowered_domain_for_test(
    ...
    // Witness proofs replay already-emitted Plutus `Data`; generic value-decoder
    // obligations apply to universal domain claims, not concrete replay verdicts.
    let include_input_bridge_assessment = !matches!(mode, CompatibilityMode::WitnessCheck);
    ...
    if include_input_bridge_assessment {
    ...
    let selected_assessment = if trust_profile == TrustProfile::Production {
        production_assessment
    } else {
        let mut assessment = assess_domain_compatibility(
            mode,
            Some(test.return_mode.clone()),
            trust_profile,
            &domain,
        ...
        assessment
            .diagnostics
            .extend(production_assessment.diagnostics);
        assessment
    };
    domain.compatibility_diagnostics = selected_assessment.diagnostics;
    if let Some(profile) = &structured_profile {
        domain
            .compatibility_diagnostics
...
fn read_bounded_command_output<R: Read>(mut reader: R) -> std::io::Result<Vec<u8>> {
    ...
    loop {
        ...
        if tail.len() > MAX_RAW_OUTPUT_TAIL_BYTES {
            let keep_from = tail.len() - MAX_RAW_OUTPUT_TAIL_BYTES;
    ...
    if total > tail.len() {
        let prefix = format!(
            "[aiken verify truncated command stream before JSON capture: retained last {} of {} bytes]\n",
            tail.len(),
            total
        );
        let mut with_notice = prefix.into_bytes();
        with_notice.extend(tail);
        Ok(with_notice)
    } else {
        Ok(tail)
    }
...
fn equivalence_theorem_name_for_entry(entry: &ManifestEntry) -> Option<String> {
    entry
...
/// on the returned `VerifyResult`. Command capture itself is first bounded
/// to `MAX_RAW_OUTPUT_TAIL_BYTES`, so a verbose solver cannot grow memory
/// without bound; `0` disables only the final JSON/log tail truncation.
pub fn run_proofs(
    out_dir: &Path,
    timeout_secs: u64,
    ...
    fn run_command_with_timeout(
        ...
        // fill OS pipe buffers before process exit. Each reader retains only a
        // bounded tail, so a verbose solver cannot force unbounded memory growth
        // before `CapturedOutput` applies the user-facing JSON cap.
        let stdout_reader = thread::spawn(move || read_bounded_command_output(&mut child_stdout));
        let stderr_reader = thread::spawn(move || read_bounded_command_output(&mut child_stderr));
    ...
    if dep_timed_out {
        let theorem_results = manifest
            ...
            .flat_map(|entry| {
                ...
                if let Some(equivalence_theorem_name) = equivalence_theorem_name_for_entry(entry) {
    ...
    for entry in &manifest.tests {
        ...
        let equivalence_theorem_name = equivalence_theorem_name_for_entry(entry);
...
pub fn generate_lean_workspace_with_cache(
    ...
    for test in tests {
        ...
        //   - `ProofCaveat::Witness(_)` — witness-only proof. Surfaces as
        //     `WitnessProved`.
        ...
        let (proof_content, proof_caveat) = match generate_proof_file(
            ...
            Ok((proof_content, proof_caveat)) => {
                ...
                let proof_caveat = if matches!(&proof_caveat, ProofCaveat::Witness(_)) {
                    // Preflight compatibility is computed before the proof route is known
                    // and describes the universal route. Witness proofs are assessed under
                    // `WitnessCheck` below, so keep their manifest verdict witness-only.
                    proof_caveat
                } else if let Some(note) = preflight_block_note {
                    append_partial_caveat_note(proof_caveat, note)
                } else {
                    proof_caveat
                };
...
fn lean_zero_bytestring_literal_of_len(len: usize) -> String {
    ...
    let mut acc = "PlutusCore.ByteString.emptyByteString".to_string();
    for _ in 0..len {
...
fn parse_top_level_pair(expr: &str) -> Result<(&str, &str), String> {
    let trimmed = expr.trim();
    let inner = trimmed
        .strip_prefix('(')
        .and_then(|value| value.strip_suffix(')'))
        .ok_or_else(|| format!("expected parenthesized pair, got {expr}"))?;
    let mut depth_paren = 0usize;
    let mut depth_bracket = 0usize;
    for (idx, ch) in inner.char_indices() {
        match ch {
            ...
            ',' if depth_paren == 0 && depth_bracket == 0 => {
                ...
                let right = inner[idx + ch.len_utf8()..].trim();
                if left.is_empty() || right.is_empty() {
                    return Err(format!("expected non-empty map pair, got {expr}"));
                }
    ...
    Err(format!("expected map pair separator, got {expr}"))
}
...
fn parse_inline_plutus_data(expr: &str) -> Result<PlutusData, String> {
    ...
    if let Some(rest) = trimmed.strip_prefix("Data.Map") {
        let inner = strip_bracketed_list(rest)
            .ok_or_else(|| format!("expected Data.Map [...], got {trimmed}"))?;
        let entries = split_top_level_list_items(inner)
            .into_iter()
            .map(|item| {
                let (key, value) = parse_top_level_pair(item)?;
                Ok((
                    parse_inline_plutus_data(key)?,
                    parse_inline_plutus_data(value)?,
                ))
            })
            .collect::<Result<Vec<_>, String>>()?;
        return Ok(PlutusData::Map(KeyValuePairs::Def(entries)));
    }
...
fn replay_counterexample_through_property(
    ...
    let flat_hex = fs::read_to_string(flat_path).map_err(|error| {
        format!(
            "read_failed: failed to read property flat file {}: {error}",
            flat_path.display()
        )
    })?;
    let bytes = flat_hex_bytes("property counterexample replay", flat_hex.trim())
        .map_err(|error| format!("harness_failed: {error}"))?;
    let program = Program::<DeBruijn>::from_flat(&bytes).map_err(|error| {
        format!(
            "harness_failed: failed to decode property flat file {}: {error}",
            flat_path.display()
        )
    })?;
    let eval_result = program.apply_data(input_data.clone()).eval(ex_budget);
...
fn counterexample_note(
    ...
    match classification {
        ...
        CounterexampleClassification::Potential => {
            if replay_status == CounterexampleReplayStatus::Confirmed {
                let outcome = property_outcome
                    .map(|value| value.to_string())
                    .unwrap_or_else(|| "unknown".to_string());
                format!(
                    "SMT falsified the generated theorem over an {} domain and replay observed {outcome} through the exported property harness, but the domain is over-approximate. This remains a potential counterexample because replay does not prove generator reachability.",
                    domain_precision
                )
            } else if replay_attempted {
                let replay_detail = match replay_status {
                    CounterexampleReplayStatus::ReadFailed => {
                        "the property artifact could not be read"
                    }
                    CounterexampleReplayStatus::HarnessFailed => {
                        "the property harness could not be decoded or executed"
                    }
                    CounterexampleReplayStatus::DecodeFailed => {
                        "the model could not be decoded into the property input type"
                    }
                    CounterexampleReplayStatus::ReplayFailed => {
                        "replay ran but did not refute the theorem"
                    }
                    CounterexampleReplayStatus::NotAttempted => "replay was not attempted",
                    CounterexampleReplayStatus::Confirmed => unreachable!("handled above"),
                };
                format!(
                    "SMT falsified the generated theorem over an {} domain, but {replay_detail}. This remains a potential counterexample rather than a confirmed generated Aiken input.",
                    domain_precision
                )
            } else {
                format!(
                    "SMT falsified the generated theorem over an {} domain. Replay has not completed yet, so this remains a potential counterexample rather than a confirmed generated Aiken input.",
                    domain_precision
                )
            }
        }
...
fn build_solver_counterexample(
        ...
        && matches!(test_mode, ManifestTestMode::Normal | ManifestTestMode::Fail)
    {
        ...
        match replay_counterexample_through_property(
            &flat_path,
            &replay_data,
            return_mode.clone(),
            context.ex_budget,
            ...
            Err(error) => {
                replay_status = if error.starts_with("read_failed:") {
                    CounterexampleReplayStatus::ReadFailed
                } else {
                    CounterexampleReplayStatus::HarnessFailed
                };
    ...
    let classification = if replay_status == CounterexampleReplayStatus::Confirmed
        && domain.precision != DomainPrecision::OverApprox
    {
        CounterexampleClassification::ConfirmedByReplay
...
impl<'a> TransitionDomainShapeState<'a> {
    ...
    fn predicate_for_data_with_schema(&mut self, type_name: &str) -> Option<String> {
        ...
        match build_exported_data_shape_predicates_into(
            self.test_name,
            &owned_schema,
            ...
            Err(_) => Some("(fun _ => False)".to_string()),
        }
    }
...
fn build_exported_data_shape_witness_from_declaration_inner(
    test_name: &str,
    schema: &ExportedDataSchema,
    declaration: &SchemaDeclaration<SchemaData>,
    ...
    match declaration {
        SchemaDeclaration::Referenced(reference) => {
            build_exported_data_shape_witness_from_reference(test_name, schema, reference, visiting)
        }
        SchemaDeclaration::Inline(data) => {
            build_exported_data_shape_witness_from_data_inner(test_name, schema, data, visiting)
        }
    }
}
    ...
    if !visiting.insert(key.clone()) {
        return Err(unsupported_error(
            ...
            UnsupportedReason::UnsupportedFuzzerOutputType {
                test_name: test_name.to_string(),
                type_repr: format!("recursive schema witness {key}"),
            },
        ));
    }
    let result = exported_data_schema_for_reference(test_name, schema, None, reference).and_then(
        |(_resolved_schema, data)| {
            build_exported_data_shape_witness_from_data_inner(test_name, schema, data, visiting)
        },
    );
    visiting.remove(&key);
...
fn build_exported_data_shape_witness_from_data_inner(
    test_name: &str,
    schema: &ExportedDataSchema,
    data: &SchemaData,
    visiting: &mut BTreeSet<String>,
    ...
    match data {
        ...
        SchemaData::Bytes => Ok("Data.B PlutusCore.ByteString.emptyByteString".to_string()),
        SchemaData::List(SchemaItems::One(_)) => Ok("Data.List []".to_string()),
        SchemaData::List(SchemaItems::Many(items)) => {
            let values = items
                .iter()
                .map(|item| {
                    build_exported_data_shape_witness_from_declaration_inner(
                        test_name,
                        schema,
                        &item.annotated,
                        visiting,
...
fn widening_log_contains_freshening(log: &TransitionWideningLog) -> bool {
    log.iter().any(|entry| {
        matches!(
            entry.kind,
            TransitionWideningKind::DataFreshening
                | TransitionWideningKind::BoolFreshening
                | TransitionWideningKind::OpaqueSubGenerator
        )
    })
}

fn try_emit_shallow_ir_as_lean_data_without_freshening(
    ir: &aiken_lang::test_framework::ShallowIr,
    ctx: &mut ShallowIrEmitCtx,
) -> Option<(String, TransitionWideningLog)> {
    let mut preview_ctx = ctx.clone();
    let baseline_existentials = preview_ctx.existentials.len();
    let mut preview_log = Vec::new();
    let out = emit_shallow_ir_as_lean_data(ir, &mut preview_ctx, &mut preview_log);
    if !widening_log_contains_freshening(&preview_log)
        && preview_ctx.existentials.len() == baseline_existentials
    {
        *ctx = preview_ctx;
        Some((out, preview_log))
    } else {
        None
    }
}

fn try_emit_shallow_ir_as_lean_bool_without_freshening(
    ir: &aiken_lang::test_framework::ShallowIr,
    ctx: &mut ShallowIrEmitCtx,
) -> Option<(String, TransitionWideningLog)> {
    let mut preview_ctx = ctx.clone();
...
fn try_emit_shallow_ir_as_lean_integer_exact(
    ...
    match ir {
        ...
        ShallowIr::Match { subject, arms } => {
            ...
            for arm in arms {
                let guard_expr = match arm.guard.as_ref() {
                    Some(guard) => Some(try_emit_shallow_ir_as_lean_bool_exact(
                        guard,
                        &mut preview_ctx,
                    )?),
                    None => None,
                ...
                match arm.tag {
                    Some(tag) => tagged.push((tag, guard_expr, body_expr)),
            ...
            for (tag, guard, body) in tagged.into_iter().rev() {
                let condition = emit_match_arm_condition(tag, guard.as_deref());
                result = format!("(if {condition} then {body} else {result})");
            }
...
fn emit_match_arm_condition(tag: u64, guard: Option<&str>) -> String {
    let tag_check = format!("decide (tag = ({tag} : Integer))");
    match guard {
        Some(guard) => format!("({tag_check} && {guard})"),
        None => tag_check,
    }
}
...
fn emit_shallow_ir_as_lean_data(
    ...
    match ir {
        ...
        ShallowIr::If {
            ...
            let mut preview_ctx = ctx.clone();
            let Some((cond_expr, mut cond_log)) =
                try_emit_shallow_ir_as_lean_bool_without_freshening(cond, &mut preview_ctx)
            else {
                push_widening(
                    unsupported_log,
                    TransitionWideningKind::DataFreshening,
            ...
            let Some((then_expr, mut then_log)) =
                try_emit_shallow_ir_as_lean_data_without_freshening(then_branch, &mut preview_ctx)
            else {
                push_widening(
                    unsupported_log,
                    TransitionWideningKind::DataFreshening,
            ...
            let Some((else_expr, mut else_log)) =
                try_emit_shallow_ir_as_lean_data_without_freshening(else_branch, &mut preview_ctx)
            else {
                push_widening(
                    unsupported_log,
                    TransitionWideningKind::DataFreshening,
            ...
            *ctx = preview_ctx;
            unsupported_log.append(&mut cond_log);
            unsupported_log.append(&mut then_log);
            unsupported_log.append(&mut else_log);
                ...
                match (shallow_ir_type_hint(left), shallow_ir_type_hint(right)) {
                    ...
                    | (_, Some(aiken_lang::test_framework::ShallowIrType::List(_))) => {
                        push_widening(
                            unsupported_log,
                            TransitionWideningKind::Domain,
                            "append widened non-list operands to empty list fallback during Data emission",
                        );
                        format!(
                    ...
                    | (_, Some(aiken_lang::test_framework::ShallowIrType::String)) => {
                        push_widening(
                            unsupported_log,
                            TransitionWideningKind::Domain,
                            "append widened non-bytes operands to empty bytes fallback during Data emission",
                        );
                        format!(
...
/// These proofs currently rely on a non-sequential trace approximation and do
/// not establish a full CEK halt obligation for every reachable trace.
const TWO_PHASE_PARTIAL_NOTE: &str = "Two-phase state-machine halt proof is partial: the generated trace predicate is non-sequential and does not establish the full CEK halt obligation — see §S6";
...
fn generate_proof_file(
        ...
        TWO_PHASE_BYPASS_WARNING_EMITTED.get_or_init(|| {
            eprintln!(
                "WARNING: AIKEN_EMIT_TWO_PHASE=0 is set; two-phase proof emission disabled. \
                 Halt tests with concrete witnesses may use witness-only native_decide proofs; \
                 halt tests without witnesses are skipped rather than routed to an unsound universal theorem. \
                 Remove this variable to restore the bounded partial scenario-proof path."
            );
        });
    }
    if !two_phase_disabled {
            ...
            Ok(Some(content)) => {
                ...
                if matches!(target, VerificationTargetKind::Equivalence) {
                    return Err(generation_error(
                        GenerationErrorCategory::FallbackRequired,
                        format!(
                            "Test '{}' cannot emit an equivalence artifact from the two-phase halt fast path; handler-comparison theorem generation is blocked until the state-machine equivalence path is implemented soundly.",
                            test.name
                        ),
                    ));
                }
    ...
    // State-machine trace halt tests have two honest proof shapes:
    //
    //   1. a symbolic two-phase artifact (preferred whenever enabled and a
    //      TransitionProp is available), reported as `Partial`;
    //   2. a concrete `native_decide` witness artifact, reported as
    //      `WitnessProved`, which proves only listed fuzzer-seed inputs.
    //
    // The generic direct state-machine theorem is not a valid fallback for halt
    // tests: `reachable` is a global over-approximation of the step function, so
    // `∀ x, reachable x → proveTestsHalt prog (dataArg x)` can be false for
    // values that no compiled trace can actually produce.
    ...
    let halt_witness_available =
        is_state_machine_trace_halt_test(test) && !test.concrete_halt_witnesses.is_empty();
    let halt_witness_route_requested =
        halt_witness_available && (two_phase_disabled || !has_step_ir);

    // Universal halt proofs require an extracted transition relation when the
    // step function was analyzed. If two-phase is enabled but no TransitionProp
    // exists, do not fall through to the unsound direct theorem. A witness-only
    // proof remains available only when the operator explicitly disabled the
    // two-phase path for this run.
    if is_state_machine_trace_halt_test(test)
        && has_step_ir
        && test.transition_prop_lean.is_none()
        && !halt_witness_route_requested
    {
        return Err(generation_error(
            GenerationErrorCategory::FallbackRequired,
            format!(
    ...
    // the two-phase path is disabled, halt tests with step IR + TransitionProp
    // but no concrete witnesses must still be skipped rather than routed into
    // the unsound universal direct theorem.
    if is_state_machine_trace_halt_test(test)
        && has_step_ir
        && test.transition_prop_lean.is_some()
        && two_phase_disabled
        && test.concrete_halt_witnesses.is_empty()
    {
        return Err(generation_error(
            GenerationErrorCategory::FallbackRequired,
            format!(
                "Test '{}': two-phase disabled by AIKEN_EMIT_TWO_PHASE=0; no sound universal direct theorem is available and no concrete halt witnesses were exported.",
                test.name
            ),
        ));
    }

    if halt_witness_route_requested {
        ensure_equivalence_has_universal_proof(
            target,
            &test.name,
        ...
        .map(|content| {
            ...
            let plural = if instances == 1 { "" } else { "es" };
            let instance_plural = if instances == 1 { "" } else { "s" };
            (
                content,
                ProofCaveat::Witness(WitnessProofNote {
                    instances,
                    witnesses,
                    note: format!(
                        "state-machine halt test '{}' verified on {} concrete fuzzer-seed-{} \
                         witness{} only (witness validation proves only the listed concrete \
                         fuzzer-seed instance{}; universal proof of `∀ x, reachable x → \
                         proveTestsHalt prog (dataArg x)` is not claimed because `reachable` \
                         is a global over-approximation of the step function)",
                        test.name, instances, CONCRETE_WITNESS_BASE_SEED, plural, instance_plural,
                    ),
                }),
            )
        });
...
fn generate_state_machine_halt_proof_file(
    ...
    if test.concrete_halt_witnesses.is_empty() {
        return Err(generation_error(
            GenerationErrorCategory::FallbackRequired,
            format!(
                "Test '{}' is a state-machine trace halt test (Void + FailImmediately), \
                 but the compiler could not synthesize any concrete fuzzer witness that \
                 drives the test body to a halting state within the attempt budget. \
                 Skipping: no sound theorem can be emitted until a witness is available \
                 or a universal two-phase proof can be emitted.",
                test.name
            ),
        ));
    }

    // Decode CBOR hex witnesses into structural `PlutusData` values. These
    // witnesses are compiler-emitted artifacts, so invalid hex/CBOR means the
    // verifier cannot honestly claim even a concrete-instance proof.
    ...
    // Cardano-style fuzzer outputs can embed many multi-byte ByteString
    // literals whose cons-chains blow past Lean's default `maxRecDepth` of 512.
    content.push_str("set_option maxRecDepth 16384\n\n");
    content.push_str(
        "-- Concrete fuzzer-generated halt witnesses.\n\
         -- Each `sample_<i>` is a Plutus `Data` value produced by running the\n\
         -- test's fuzzer with seed 42+i; the test body halts on this input.\n\
         -- The corresponding `native_decide` theorem reduces the closed term\n\
         -- `proveTestsHalt prog (dataArg sample_<i>)` without SMT.\n\n",
    );

    // Bool reflection of the void success classifier. `proveTestsHalt` unfolds
    // to a `Prop`-valued classifier; the Bool version lets `native_decide`
    // reduce the closed CEK execution and then rewrite back to the public theorem.
    content.push_str(
        "/-- Bool-valued reflection of `AikenVerify.Utils.PropertySucceeds .void`. -/\n\
         private def propertySucceedsVoidBool : TestEvalOutcome → Bool\n  \
           | .returnsVoid => true\n  \
           | _            => false\n\n\
         private theorem propertySucceedsVoidBool_iff_propertySucceedsVoid (out : TestEvalOutcome) :\n  \
             propertySucceedsVoidBool out = true ↔ PropertySucceeds .void out := by\n  \
           cases out <;> simp [propertySucceedsVoidBool, PropertySucceeds]\n\n",
    );
    ...
    for i in 0..witnesses.len() {
        content.push_str(&format!(
            "theorem {lean_test_name}_instance_{i} :\n  \
             ...
             rw [← propertySucceedsVoidBool_iff_propertySucceedsVoid]\n  \
             native_decide\n\n"
        ));
    }
...
mod result_parser;
pub use result_parser::{VERIFY_SUMMARY_VERSION, VerifyParseContext, parse_verify_results};
use result_parser::{
