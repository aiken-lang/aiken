...
fn no_proofs_summary(
    ...
    verify::VerifySummary {
        ...
        elapsed_ms: None,
        command_success: !skipped_without_allow,
    }
}
...
error: Foo.lean:15:5: Tactic `blaster` failed";
    ...
    fn command_success_in_json_reflects_skip_failure() {
        ...
        let manifest = verify::GeneratedManifest {
            version: "1.0.0".to_string(),
            tests: vec![verify::ManifestEntry {
                id: "example_test_ok".to_string(),
                aiken_module: "example".to_string(),
                aiken_name: "test_ok".to_string(),
                lean_module: "Example.TestOk".to_string(),
                lean_theorem: "example_test_ok".to_string(),
                lean_file: "Example/TestOk.lean".to_string(),
                flat_file: "example_test_ok.flat".to_string(),
        ...
        let raw = verify::VerifyResult {
            success: true,
            stdout: String::new(),
            stderr: String::new(),
            exit_code: Some(0),
            theorem_results: Some(vec![verify::TheoremResult {
                test_name: "example.test_ok".to_string(),
                theorem_name: "Example.TestOk".to_string(),
                status: verify::ProofStatus::Proved,
            }]),
        };
    ...
    fn verify_run_generate_only_supports_non_default_targets() {
        ...
        for target in [
            ...
            // The fixture uses a `todo` fuzzer which has opaque semantics.
            // With sampled-domain fallback removed, skip unsupported tests.
            run_options.skip_unsupported = true;
            run_options.allow_skips = true;
            ...
            // With sampled-domain fallback removed, the fixture's todo fuzzer
            // has opaque semantics and gets skipped. The manifest should exist
            // even if no tests were generated.
